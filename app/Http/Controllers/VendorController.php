...
class VendorController extends Controller
{
    ...
    public function cari(Request $req){
        $datacari=Vendor::where('nama_mitra','like','%'.$req->cari.'%')
        ->orWhere('email_mitra','like','%'.$req->cari.'%')
        ->orWhere('kontak','like','%'.$req->cari.'%')
        ->get();
        
        if (!$datacari->isEmpty()){
            return view('carivendor.carivendor',['data'=>$datacari]);
            dd($datacari);
        }else{
            return view('carivendor.carivendor',['data'=>'kosong']);
        }
    }
}
